    ...
    public class UsuarioRepository : BaseRepository<Usuario>, IUsuarioRepository
    {
            ...
            using (var reader = await _dbHelper.ExecuteReaderAsync(query))
            {
                while (await reader.ReadAsync())
                {
                    usuarios.Add(new Usuario
                    {
                        ...
                        Rol = ConvertirRol(reader.GetString(3)),
        ...
        public override async Task<int> CreateAsync(Usuario usuario)
        {
            ...
            var parameters = new MySqlParameter[]
            {
                ...
                new MySqlParameter("@Rol", ConvertirRolParaBaseDeDatos(usuario.Rol)),
        ...
        public async Task<IEnumerable<Usuario>> GetByRolAsync(RolUsuario rol)
        {
            ...
            var parameters = new MySqlParameter[] { new MySqlParameter("@Rol", ConvertirRolParaBaseDeDatos(rol)) };
        ...
        public override async Task<IEnumerable<Usuario>> GetPagedAsync(int page, int pageSize, string? searchTerm = null, bool soloDisponibles = false)
        {
            _logger.LogInformation("Obteniendo usuarios paginados - Pagina: {Page}, Tamano: {PageSize}", page, pageSize);
            ...
            {
                query += " WHERE nombre_completo LIKE @SearchTerm OR email LIKE @SearchTerm OR rol LIKE @SearchTerm";
                parameters.Add(new MySqlParameter("@SearchTerm", $"%{searchTerm}%"));
            }

            query += " ORDER BY id_usuario DESC LIMIT @PageSize OFFSET @Offset";
        ...
        // obtiene el total de usuarios para paginacion
        public override async Task<int> GetTotalCountAsync(string? searchTerm = null, bool soloDisponibles = false)
        {
            _logger.LogInformation("Obteniendo total de usuarios");

            var query = "SELECT COUNT(1) FROM usuario";
            var parameters = new List<MySqlParameter>();
        ...
        private static RolUsuario ConvertirRol(string rol)
        {
            return Enum.TryParse<RolUsuario>(rol, ignoreCase: true, out var rolUsuario)
                ? rolUsuario
                : RolUsuario.Empleado;
        }

        private static string ConvertirRolParaBaseDeDatos(RolUsuario rol)
        {
            return rol.ToString().ToUpperInvariant();
        }
    }
}